...
namespace RelicHunting
{
	...
	public static class Dialog_BeginLordJob_DrawQualityDescription_Patch
	{
		...
		{
            FieldInfo ritualField = typeof(Dialog_BeginRitual).GetField("ritual", BindingFlags.NonPublic | BindingFlags.Instance);
            Precept_Ritual ritual = (Precept_Ritual)ritualField.GetValue(__instance);

            if (__instance is Dialog_BeginRitual beginRitual
				&& ritual?.attachableOutcomeEffect?.Worker
				is RitualAttachableOutcomeEffectWorker_DiscoverRelics)
			{
				...
				if (ideos.TryGetValue(ritual, out var ideoPreceptTuple) is false)
				{
					ideoPreceptTuple = (Faction.OfPlayer.ideos.PrimaryIdeo, null);
					ideos[ritual] = ideoPreceptTuple;
				}
				...
				if (ideos.TryGetValue(ritual, out var ideoPreceptTuple2)
				...
				if (Widgets.ButtonImage(rect7.ContractedBy(2f), icon))
				{
					...
					foreach (Precept_Relic relicPrecept in availableRelics)
					{
						...
						Action action = delegate
						{
							...
							ideos[ritual] = (ideos[ritual].Item1, selectedRelicPrecept);
						};
